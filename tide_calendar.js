# tide_calendar.js
/**
 * 🌊 Tide Calendar – Smart Fishing Planner (Node.js Edition)
...
const DEFAULT_HARMONICS = { M2: 1.2, S2: 0.4, N2: 0.2, K2: 0.1, M4: 0.1 };
const LUNAR_CYCLE_DAYS = 29.530588853;
const TIDE_PERIOD_HOURS = 12.4206012;
...
class TideStation {
    constructor(name, latitude = 0, longitude = 0, harmonics = null, tzOffset = 0) {
        this.name = name;
        this.latitude = latitude;
        this.longitude = longitude;
        this.harmonics = harmonics || { ...DEFAULT_HARMONICS };
...
class TidePredictor {
    ...
    _daysSinceEpoch(dt) {
        return (dt - this.epoch) / (1000 * 60 * 60 * 24);
    }

    _moonPhase(dt) {
        const days = this._daysSinceEpoch(dt);
        return (days / LUNAR_CYCLE_DAYS) % 1.0;
    }
    ...
    getExtrema(dt, hours = 12.5) {
        ...
        while (current <= end) {
            ...
            if ((h - prevH) * (nextH - h) < 0) {
                ...
                if (ext) {
                    const [hExt, tExt] = ext;
                    const typ = hExt > 1.0 ? 'High' : 'Low';
                    results.push({ time: tExt, height: hExt, type: typ });
                }
            }
            prevH = h;
            current = new Date(current.getTime() + step);
...
function fishingForecast(height, moonPhase, dt) {
...
function drawTideChart(predictor, dt, hours = 24, width = 60, height = 10) {
    const start = dt;
...
class ProfileManager {
    ...
    addProfile(name, lat, lon, harmonics = null) {
